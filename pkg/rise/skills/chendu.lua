...
Fk:loadTranslationTable{
  ...
  [':chendu'] = '锁定技，当你的牌因使用、打出或弃置进入弃牌堆后，若数量大于你的体力值，你将这些牌分配给其他角色（若不为你的回合，则选择的角色必须包含当前回合角色）。',
}
...
chendu:addEffect(fk.AfterCardsMove, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    local all_cards = table.simpleClone(event:getCostData(skill))
    ...
    if to == player then
      ...
      if #cards == 0 then
        ...
        room:moveCardTo(ids, Card.PlayerHand, to, fk.ReasonGive, chendu.name, nil, true, player.id)
      else
        ...
        room:doYiji(list, player.id, chendu.name)
      end
    end
  end,
})
