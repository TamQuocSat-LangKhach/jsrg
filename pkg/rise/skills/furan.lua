...
furan:addEffect(fk.Damaged, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(furan.name) and data.from and
...
furan:addEffect(fk.TurnEnd, {
  ...
  on_use = function (skill, event, target, player, data)
    player.room:recover{
      who = player,
      num = math.min(player:usedSkillTimes(furan.name, Player.HistoryTurn), player:getLostHp()),
      recoverBy = player,
